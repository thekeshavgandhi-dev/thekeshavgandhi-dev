...
function buildBubbleShooterSVG({ data, width = 1200, height = 340, theme, speedMul = 1, noContributionColor = '#ebedf0', transparent = true }) {
  ...
  <rect id="cycleTimer" x="-10" y="-10" width="1" height="1" fill="none">
    ...
    <!-- Shooter: Rocket -->
  <g id="shooter">
    <!-- Rocket body (rounded top, extends down to nozzle) -->
    <rect x="${shooterX - 7}" y="${shooterY - 24}" width="14" height="24" rx="7" fill="${theme.shooter}" />
    
    <!-- Left fin -->
    <polygon points="${shooterX - 7},${shooterY - 4} ${shooterX - 17},${shooterY + 2} ${shooterX - 7},${shooterY + 2}" fill="${theme.shooter}" />
    
    <!-- Right fin -->
    <polygon points="${shooterX + 7},${shooterY - 4} ${shooterX + 17},${shooterY + 2} ${shooterX + 7},${shooterY + 2}" fill="${theme.shooter}" />
    
    <!-- Window (cabin) -->
    <circle cx="${shooterX}" cy="${shooterY - 18}" r="3.5" fill="#ffffff" opacity="0.9" />
    
    <!-- Flame (static, below nozzle) -->
    <polygon points="${shooterX - 4},${shooterY} ${shooterX + 4},${shooterY} ${shooterX},${shooterY + 9}" fill="#ff4500" opacity="0.8" />
    <polygon points="${shooterX - 2.5},${shooterY} ${shooterX + 2.5},${shooterY} ${shooterX},${shooterY + 6}" fill="#ffd700" opacity="0.9" />
  </g>
